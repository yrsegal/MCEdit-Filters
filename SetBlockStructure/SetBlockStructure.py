...
mustPlaceLast = [6, 8, 9, 10, 11, 12, 13, 26, 27, 28, 31, 32, 34, 36, 37, 38, 39, 40, 50, 51, 55, 59, 63, 64, 65, 66, 68, 69, 70, 71, 72, 75, 76, 77, 78, 81, 83, 90, 93, 94, 96, 104, 105, 106, 111, 115, 119, 122, 123, 127, 131, 140, 141, 142, 143, 145, 147, 148, 149, 150, 151, 157, 171, 175]
...
displayName = "SetBlock Structure"
...
def perform(level, box, options):
	...
	if options["Create Builder"]:
		buildsFirst = buildsFirstStructureBuilder(box)[0]
		buildsLast = buildsFirstStructureBuilder(box)[1]
		createCmdBlock(level, box, options, buildsFirst, buildsLast, 0, "2", "Build")
	
	if options["Create Deleter"]:
		builds = deleteStructureBuilder(box)
		createCmdBlock(level, box, options, builds, [], 2, "4", "Delete")

def createCmdBlock(level, box, options, buildsFirst, buildsLast, cdz, color, text):
	...
	curlyBrackets = []
	isHelperCart = False
	helperCartCurlyBrackets = []
	i = 0
	delayGameTicks = 1

	for (bposX, bposY, bposZ, bblock, bdata, btileEntity) in buildsFirst:
		if options["Use relative coordinates"]:
			bposX = "~"+str(bposX-rsx)
			bposY = "~"+str(bposY-rsy-4)
			bposZ = "~"+str(bposZ-rsz)
		else:
			if bposX < 0:
				bposX = bposX-1
		...
		if i <= 9:
			dataTags.append("id:MinecartCommandBlock,Command:setblock "+bposX+" "+bposY+" "+bposZ+" "+bID+" "+str(bdata)+" replace ")
			if btileEntity and options["Include TileEntity Data"]:
				dataTags.append(tagCode(btileEntity))
			else:
				dataTags.append(",")
			dataTags.append("Riding:{")

			if isHelperCart == False:
				curlyBrackets.append("}")
			else:
				helperCartCurlyBrackets.append("}")

			i = i+1

		else:
			if isHelperCart == True:
				dataTags.append("".join(helperCartCurlyBrackets)+",Riding:{")
				curlyBrackets.append("}")
			else:
				isHelperCart = True

			dataTags.append("id:MinecartCommandBlock,Command:summon MinecartCommandBlock ~ ~ ~ {id:MinecartCommandBlock,Command:setblock "+bposX+" "+bposY+" "+bposZ+" "+bID+" "+str(bdata)+" replace ")
			if btileEntity and options["Include TileEntity Data"]:
				dataTags.append(tagCode(btileEntity))
			else:
				dataTags.append(",")
			...
			delayGameTicks = delayGameTicks+1

	if buildsLast != []:

		# temporarily
		for (bposX, bposY, bposZ, bblock, bdata, btileEntity) in buildsLast:
			if options["Use relative coordinates"]:
				bposX = "~"+str(bposX-rsx)
				bposY = "~"+str(bposY-rsy-4)
				bposZ = "~"+str(bposZ-rsz)
	...
	mainCommand = "summon MinecartCommandBlock ~ ~3 ~"+str(-cdz)+" {"+"".join(dataTags)+"id:MinecartCommandBlock,Command:summon Arrow ~ ~0.2 ~ {damage:10000.0}}"+"".join(helperCartCurlyBrackets)+"".join(curlyBrackets)
	print "Command: "+mainCommand

	# Create Cmd Block
	level.setBlockAt(rsx, rsy, rsz+cdz, 137)
	cmd = cmdBlock((rsx, rsy, rsz+cdz), mainCommand)
	chunk = getChunk(rsx, rsz+cdz)
	chunk.TileEntities.append(cmd)
	chunk.dirty = True

	# Create other Blocks
	level.setBlockAt(rsx+1, rsy, rsz+cdz, 77) # Button
	level.setBlockDataAt(rsx+1, rsy, rsz+cdz, 1) # Button Direction
	level.setBlockAt(rsx, rsy+3, rsz, 157) # Activator Rail
	level.setBlockDataAt(rsx, rsy+3, rsz, 9) # Power Activator Rail
	level.setBlockAt(rsx, rsy+2, rsz, 81) # Cactus
	level.setBlockAt(rsx, rsy+1, rsz, 12) # Sand
	level.setBlockAt(rsx, rsy+3, rsz+1, 1) # Stone 1
	level.setBlockAt(rsx, rsy+3, rsz-1, 1) # Stone 2
	level.setBlockAt(rsx+1, rsy+3, rsz, 1) # Stone 3
	level.setBlockAt(rsx-1, rsy+3, rsz, 1) # Stone 4
	level.setBlockAt(rsx, rsy, rsz+cdz+1, 68) # Sign
	level.setBlockDataAt(rsx, rsy, rsz+cdz+1, 3) # Sign Rotation
	chunk = getChunk(rsx, rsz)
	chunk.dirty = True
	chunk = getChunk(rsx+1, rsz+cdz)
	chunk.dirty = True
	chunk = getChunk(rsx, rsz+cdz)
	chunk.dirty = True

	# Create Sign
	delayRedstoneTicks = delayGameTicks

	sign = signTileEntity(rsx, rsy, rsz+cdz+1, "§lSetBlock", "§8§l"+str(delayRedstoneTicks)+" Ticks", "§"+color+text+"s", "§"+color+"§l"+str(len(buildsFirst)+len(buildsLast))+" Blocks")
	chunk.TileEntities.append(sign)

def buildsFirstStructureBuilder(box):
	buildsFirst = []
	buildsLast = []
	alreadyBuilt = []
	for x in xrange(box.minx, box.maxx):
		for z in xrange(box.minz, box.maxz):
			for y in xrange(box.maxy-1, box.miny-1, -1):
				block = blockAt(x, y, z)
				if block != 0 and (x, y, z) not in alreadyBuilt:
					if block not in mustPlaceLast:
						buildsFirst.append((x, y, z, block, dataAt(x, y, z), tileEntityAt(x, y, z)))
					else:
						i = 0

						while blockAt(x, y-i, z) in mustPlaceLast and y-i >= box.miny:
							i = i+1
						while i >= 0:
							if blockAt(x, y-i, z) in mustPlaceLast:
								buildsLast.append((x, y-i, z, blockAt(x, y-i, z), dataAt(x, y-i, z), tileEntityAt(x, y-i, z)))
							else:
								buildsFirst.append((x, y-i, z, blockAt(x, y-i, z), dataAt(x, y-i, z), tileEntityAt(x, y-i, z)))
							alreadyBuilt.append((x, y-i, z))
							i = i-1

	return (buildsFirst, buildsLast)

def deleteStructureBuilder(box):
	builds = []
	for x in xrange(box.minx, box.maxx):
		for z in xrange(box.minz, box.maxz):
			for y in xrange(box.maxy-1, box.miny-1, -1):
